...
class TestLlmExtended:
    ...
    def test_llm_stream_non_groq_no_api_base(self):
        ...
        with patch('app.core.llm.completion', return_value=iter([])) as mock, patch('app.core.llm.os.getenv', return_value='https://api.groq.com'):
            llm_stream(ag, [], [], lambda t: None)
    ...
    def test_llm_stream_groq_no_env_no_api_base(self):
        ...
        from app.core.agent import Agent
        ag = Agent(name='x', instructions='hi', model='groq/llama')
        with patch('app.core.llm.completion', return_value=iter([])) as mock, patch('app.core.llm.os.getenv', return_value=None):
            llm_stream(ag, [], [], lambda t: None)
            assert 'api_base' not in mock.call_args[1]
    ...
    def test_llm_stream_groq_with_api_base(self):
        ...
        with patch('app.core.llm.completion', return_value=iter([])) as mock, patch('app.core.llm.os.getenv', side_effect=getenv_side):
            llm_stream(ag, [], [], lambda t: None)
...
class TestRuntimeExtended:
    ...
    def test_run_swarm_mixed_handoff_and_normal_tool(self):
        ...
        def handoff_fn(details: str):
            return handoff('writer')

        def normal_tool(x: str):
            return f'normal:{x}'
        ...
        session = self.make_session()
        calls = [ToolCall(id='c1', name='normal_tool', arguments={'x': 'hello'}), ToolCall(id='c2', name='handoff_fn', arguments={'details': 'go'})]
    ...
    def test_run_swarm_truncation_boundary(self):
        ...
        def exact_tool(x: str):
            return 'A' * MAX_TOOL_RESULT_CHARS
    ...
    def test_run_swarm_truncation_plus_one(self):
        ...
        def big(x: str):
            return 'A' * (MAX_TOOL_RESULT_CHARS + 1)
