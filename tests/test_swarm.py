...
class TestLlm:
    ...
    def test_llm_stream_simple_content(self):
        ...
        chunks = [make_litellm_chunk(content='Hello '), make_litellm_chunk(content='world'), make_litellm_chunk(content=None)]
        on_token_calls = []
    ...
    def test_llm_stream_sets_groq_api_base(self):
        ...
        with patch('app.core.llm.completion', return_value=iter([])) as mock_comp, patch('app.core.llm.os.getenv') as mock_getenv:
...
class TestRuntime:
    ...
    def test_run_swarm_last_handoff_wins(self):
        ...
        session = self.make_session()
        calls = [ToolCall(id='c1', name='h1', arguments={'details': 'a'}), ToolCall(id='c2', name='h2', arguments={'details': 'b'})]
...
class TestWebSearch:
    ...
    def test_web_search_uses_tavily_when_key_set(self):
        ...
        with patch('app.tools.web_search.os.getenv', return_value='fake_key'), patch('app.tools.web_search._tavily', return_value=[{'title': 't', 'url': 'u', 'snippet': 's'}]) as mock_t, patch('app.tools.web_search._duckduckgo') as mock_ddg:
    ...
    def test_web_search_uses_duckduckgo_when_no_key(self):
        from app.tools import web_search as ws_mod
        with patch('app.tools.web_search.os.getenv', return_value=None), patch('app.tools.web_search._duckduckgo', return_value=[{'title': 't', 'url': 'u', 'snippet': 's'}]) as mock_ddg, patch('app.tools.web_search._tavily') as mock_t:
    ...
    def test_web_search_clamps_max_results(self):
        from app.tools import web_search as ws_mod
        with patch('app.tools.web_search.os.getenv', return_value=None), patch('app.tools.web_search._duckduckgo', return_value=[{'title': 't', 'url': 'u', 'snippet': 's'}]) as mock_ddg:
    ...
    def test_web_search_exception_returns_error_json(self):
        from app.tools import web_search as ws_mod
        with patch('app.tools.web_search.os.getenv', return_value=None), patch('app.tools.web_search._duckduckgo', side_effect=RuntimeError('fail')):
    ...
    def test_tavily_parsing(self):
        ...
        mock_resp = MagicMock()
        mock_resp.json.return_value = {'results': [{'title': 'T', 'url': 'U', 'content': 'C'}, {'title': 'T2', 'url': 'U2', 'content': 'C2'}]}
        mock_resp.raise_for_status = MagicMock()
    ...
    def test_tavily_sends_correct_payload(self):
        ...
        mock_resp.raise_for_status = MagicMock()
        with patch('app.tools.web_search.httpx.post', return_value=mock_resp) as mock_post, patch('app.tools.web_search.os.getenv', return_value='key123'):
            _tavily('hello', 5)
