...
class TestIntegrationSwarmFlow:
    ...
    def test_full_flow_triage_to_researcher_to_writer(self):
        ...
        def handoff_researcher(details: str):
            return handoff('researcher', details)

        def handoff_writer(details: str):
            return handoff('writer', details)

        def handoff_triage(details: str):
    ...
    def test_multiple_handoffs_last_wins(self):
        ...
        def h1(d: str):
            return handoff('researcher', d)

        def h2(d: str):
            return handoff('writer', d)
        ...
        registry = {'triage': triage, 'researcher': researcher, 'writer': writer}
        session = self.make_session()
        calls = [ToolCall(id='c1', name='h1', arguments={'d': 'a'}), ToolCall(id='c2', name='h2', arguments={'d': 'b'})]
...
class TestConfigAndEnv:
    ...
    def test_temperature_env(self):
        ...
        try:
            ...
            import app.core.llm as llm_mod
            reload(llm_mod)
            assert abs(llm_mod.TEMPERATURE - 0.42) < 1e-06
        finally:
            if old is None:
